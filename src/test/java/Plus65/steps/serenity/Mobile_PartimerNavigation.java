...
public class Mobile_PartimerNavigation {

	Android_PartimerHomepage partimerHomepage;
	CurrentPage currentPage;
	PartimerNavBar navBar;
	ParitimerJobOffers jobOffers;
	PartimerMyJobs myJobs;
	Mobile_PartimerRegistrationPage partimerRegistrationPage;
	...
	public void shouldSeeText(String expectedText) {
		...
		{
			if(MobileDriver.driver.findElements(By.xpath("//android.widget.TextView[starts-with(@text, '"+expectedText+"')]")).size()>=1)
			{
				assertTrue(MobileDriver.driver.findElement(By.xpath("//android.widget.TextView[contains(@text, 're almost there')]")).getText().contains("You're almost there!"));
				assertTrue(MobileDriver.driver.findElement(By.xpath("//android.widget.TextView[starts-with(@text, '"+expectedText+"')]")).getText().contains(expectedText));
			}
			else
				assertThat("Registration is Failed");
		}
		else
		{
			if(MobileDriver.driver.findElements(By.xpath("//XCUIElementTypeOther[contains(@text, 're almost there')]")).size()>=1)
				assertTrue(MobileDriver.driver.findElement(By.xpath("//XCUIElementTypeOther[contains(@text, 're almost there')]")).getText().contains(expectedText));
			else
				assertThat("Registration is Failed");
		}
	}
	...
	@Step
	public void clickOnCreateNewAccountButton() throws InterruptedException {
		partimerHomepage.selectStagingEnv();
	...
	public void entersNewAccountDetails(String firstName, String lastName, String email, String password,
			String password_confirmation, String referral_code) throws InterruptedException {
	...
	@Step
	public void clicksOnCreateNowButton() throws InterruptedException {
